...
// fn receives the captures plus the docstring BODY as a plain string, with hasBody
// telling it whether the step carried one. The *godog.DocString pointer never leaves
// this function: it is dereferenced here, once, under an explicit nil check, so no
// downstream code can deref a nil docstring — the defect a package gate exists to
// catch, and which shipped once already in this repo.
...
func resolvePhrases(eng *engine.Engine) ([]contributedPhrase, error) {
	...
	for _, b := range bindings {
		re, err := regexp.Compile(b.Phrase.Pattern)
		if err != nil {
			return nil, fmt.Errorf("comparator %q contributes an uncompilable pattern %q: %w",
				b.Comparator, b.Phrase.Pattern, err)
		}
		out = append(out, contributedPhrase{
			...
			arity:    re.NumSubexp(),
			wantsDoc: declaresDocstring(b.Phrase.Pattern),
		})
	}
	return out, nil
}
