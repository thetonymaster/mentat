...
| The returned value goes straight to `Compare` | Mentat does not inspect, copy, or re-shape it. It is handed to `Engine.Compare` as the `Expectation`. The **one** exception is the nil check below — a nil test, never a shape test. |
| A nil expectation with a nil error is REFUSED | Added 2026-09-10 (CodeRabbit, PR #39). A parser claiming success while returning `nil` is a comparator bug, but forwarding the nil would let a comparator that tolerates nil return a **passing** verdict for a step that asserted nothing. The step returns a descriptive error naming the comparator instead. This catches an *untyped* nil only: a typed nil pointer is a non-nil interface, reaches `Compare`, and is the comparator's own type assertion to make. A zero-valued but non-nil expectation is legitimate and passes through — the guard rejects nil, not emptiness. |
