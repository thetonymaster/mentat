...
- [ ] T025b [US5] **POST-MERGE, still required**: dispatch on the default branch and watch the dispatched run explicitly — `gh workflow run nightly-l3.yml`, resolve its id via `gh run list --workflow nightly-l3.yml --event workflow_dispatch --limit 1 --json databaseId --jq '.[0].databaseId'`, then `gh run watch "$run_id" --exit-status` (a bare `gh run watch` exits 0 even on a red run, so `&&`-chaining it reports success on exactly the failure this story exists to catch); VERIFY green at 20 runs; record the run URL via `gh run view "$run_id" --json url --jq .url`. Full snippet in [quickstart.md](./quickstart.md) V5. SC-005 is only PARTLY met until this is done — merging does not close it. If the dispatched run is red, STOP and diagnose per RULE 0 before rerunning (a red 20-run lane is exactly the signal this story exists to surface)
