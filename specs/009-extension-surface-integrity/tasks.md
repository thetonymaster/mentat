...
- [X] T025b [US5] **POST-MERGE, still required**: dispatch on the default branch and watch THAT run. Run the snippet in [quickstart.md](./quickstart.md) V5 verbatim — it captures the pre-dispatch run id, polls until a newer `workflow_dispatch` run appears, and watches it with `--exit-status`. Do not shortcut it to `gh workflow run … && gh run watch`: a bare `gh run watch` exits 0 even on a red run, so that chain reports success on exactly the failure this story exists to catch SC-005 is only PARTLY met until this is done — merging does not close it. If the dispatched run is red, STOP and diagnose per RULE 0 before rerunning (a red 20-run lane is exactly the signal this story exists to surface)

**Checkpoint**: SC-005 **fully met**. The workflow exists, the 20-run threshold is proven locally (T025a), and the post-merge dispatched run is proven green on `main` (T025b): [run 34388981631](https://github.com/thetonymaster/mentat/actions/runs/34388981631), `event: workflow_dispatch`, `conclusion: success`, completed in 2m31s.
