...
**Why this priority**: Documentation debt, not a live defect — but it is the stated precondition for the next features (010–014), and today the "six seams" are defined differently in two places while one comparator's internal-only status is documented nowhere.
...
- Out of scope (deferred to later specs): seam-type nameability (010 — the gap this feature verified and recorded, see `contracts/facade-nameability.md`), custom-comparator Gherkin invocation (011), comparator-contributed Gherkin phrases (012), built-in step-pattern disjointness (013), CLI/mentatctl UX (014), and adding any new seam — this feature is the precondition for those. *(Renumbered 2026-09-09: 010 was originally earmarked for the Gherkin item, but the nameability gap this feature surfaced took the slot, and `docs/extending/stability.md` boundary 4 already pointed there.)* *(Renumbered again 2026-09-10 by 011's D1: 011 shipped only Option A — one generic `stepDefs` row plus an optional `ExpectationParser` seam — and deferred Option B, comparator-contributed Gherkin phrases, to 012, which pushes CLI/mentatctl UX to 013. See `specs/011-comparator-gherkin-invocation/spec.md` D1.)* *(Renumbered a third time 2026-09-11 by 012's convergence: 013 is now built-in step-pattern disjointness — 012 established that "the 40 built-in patterns are pairwise disjoint" is evidence from nine fixed generator fillers rather than proof, and four claims rest on it — so CLI/mentatctl UX moves to 014. See the roadmap block in `CLAUDE.md`.)*
