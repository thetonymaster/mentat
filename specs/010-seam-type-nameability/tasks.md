...
- [X] T059 **DECIDED: normalize field types to the facade's vocabulary.** Option (a) — one rendering per type, rule + test + golden updated. Implemented as **facade-name normalization**, not internal-package qualification: investigation showed method signatures ALREADY rendered facade names (`method (Correlator) Resolve(…, store TraceStore, …)`, never `core.TraceStore`), and that 11 of the 14 qualified field lines were stdlib. Qualifying uniformly with internal names would have rewritten ~130 lines *and* left fields contradicting methods; normalizing toward the facade name achieves the stated goal — one type, one rendering, zero churn on a package move — in **8 lines**. Alias and const declaration lines are deliberately excluded, since naming the target is their purpose. Verified the rename-as-drift property survives: it moved from N inconsistent field lines to the single canonical alias line (`type AggregateDetail = core.AggregateDetail`), which a rename must change or the code will not compile. Two new pinning rows added — the same type rendering identically from another package, and the alias line keeping its internal target
