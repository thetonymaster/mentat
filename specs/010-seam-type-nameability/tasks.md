...
- [X] T056 Run `make ci` green, then `go-reviewer` in `gate` mode. **Ran; verdict was BLOCK; blocks cleared.** The audit found four uncovered error paths — the sixth seam had shipped without the error-path table rows its five siblings all have (`build.go` nil-factory / factory-error / isNilSeam, and `run.go`'s facade-level nil-factory guard). Correct finding: the whole reporter branch of `applyExtras` measured count=0, because the facade tests that exercise it live in the root package and never reach `internal/engine`'s own profile. Fixed in `e3ae392`; all four re-measured at count=1. Two FLAG items also fixed (the degraded `(registered: )` message, a CHANGELOG double blank line). The audit independently verified D1–D5 as implemented, regenerated the mocks byte-identically, confirmed the report goldens are 0-line-diff since `d21ebab`, and found zero AI-attribution matches across all commits. One FLAG deferred — see T059
...
- [X] T057 **DECIDED: export the extraction-mode constants.** Raised at T010. `mentat.ExtractPolicy` was nameable but `ExtractWhole`/`ExtractMarker`/`ExtractPattern` were not, so an external driver author wrote `Mode: "pattern"` as a magic string and a typo became a run-time extraction error rather than a compile error. The decision follows the `FailureKind*` precedent, which already exports its constants for exactly this reason: **a published field whose legal values are unnameable is only half published.** Added to `mentat.go` with per-constant justifications (SC-007), proven RED-then-GREEN from the facade-only test, plus `TestExtractModeConstantsAreDistinct` pinning that the three are distinct and non-empty — `ExtractWhole` deliberately is not `""`, so a caller can still tell "unset" from "explicitly whole". Surface golden: 3 const lines. `stability.md` no longer names this as an open gap
...
- [ ] T059 **DECISION NEEDED — should the surface golden qualify field types uniformly?** Raised by the gate audit. The renderer prints field types *as written in the declaring package's source*, so one type renders two ways depending on where the field lives: `field (Results)[08] JudgeTotal *core.JudgeUsage` (declared in `internal/result`) versus `field (Verdict)[04] Judge *JudgeUsage` (declared in `internal/core`). Both are `core.JudgeUsage`. Harmless today, but it means moving a type between internal packages produces golden churn that *reads* like a surface change — which is exactly what happened to `Results` in this feature. **Not a cosmetic fix**: "as written" is a documented rule (`surface-golden-v2.md:19`, `surface_test.go:700`) with its own unit test pinning it (`surface_test.go:232`), and 130 of the 144 field lines are currently unqualified, so uniform qualification rewrites most of the reviewed artifact. Changing it also risks losing the property the rule buys — a rename of the named type currently shows as drift. Options: (a) qualify uniformly and update the rule, its test and the golden; (b) leave as-is and document the churn behaviour in `stability.md` so it is not mistaken for a surface change; (c) defer to a later spec. **Not to be actioned without an explicit decision**
